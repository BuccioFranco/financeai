...
export default function MarketTicker() {
  ...
  const items = data
    ? [
        ...
        { label: '🏛️ Oficial', value: `$${fmt(data.oficial)}`, color: 'text-gray-400' },
        {
          label: '📈 IPC mensual',
          value: `${fmt(data.ipc_mensual, 1)}%`,
          color: data.ipc_mensual > 5 ? 'text-red-400' : 'text-yellow-400',
        },
        {
          label: '📊 IPC 12m',
          value: `${fmt(data.ipc_acumulada_12m, 1)}%`,
          color: 'text-orange-400',
        },
        { label: '₿ BTC', value: `USD ${fmt(data.btc_usd)}`, color: 'text-yellow-400' },
        { label: '⟠ ETH', value: `USD ${fmt(data.eth_usd)}`, color: 'text-purple-400' },
      ]
    : [{ label: 'Cargando cotizaciones...', value: '', color: 'text-gray-500' }]

  const doubled = [...items, ...items]

  return (
    <div className="bg-card border-b border-border overflow-hidden h-9 flex items-center">
